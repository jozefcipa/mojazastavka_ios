...
import {parseDistance} from "../helpers";
...
export default class Map extends Component {
    ...
    shouldComponentUpdate(nextProps) {
        ...
        if (!isEqual(nextProps.mapCenterPoints, this.props.mapCenterPoints)) {
            ...
            this.map.fitToCoordinates(nextProps.mapCenterPoints, {
                edgePadding: Constants.GEO.EDGE_PADDING,
                animated: true
            });
        }
                    ...
                    this.props.points.nearby.map((point, i) => {
                        return (
                            <MapView.Marker
                                ...
                                <Callout {...point}
                                         descriptionText={parseDistance(parseFloat(point.distance_in_meters)) + ' od Vás'}/>
                            </MapView.Marker>
                        );
                    })
                }
                    ...
                    this.props.points.destination.map((point, i) => {
                        return (
                            <MapView.Marker
                                ...
                                <Callout {...point}
                                         descriptionText={parseDistance(parseFloat(point.distance_in_meters)) + ' do cieľa'}/>
                            </MapView.Marker>
                        );
                    })
                }
