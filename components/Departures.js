                    ...
                    onPress={() => {
                        ...
                        if (props.showMenu === Constants.DEPARTURES_MENU.ENABLED) {
                            if (props.departures.length > 0) {
                                props.showDeparturesMenu(Constants.DEPARTURES_MENU.RESULTS);
                            }
                            else {
                    ...
                    props.showMenu !== Constants.DEPARTURES_MENU.HIDDEN && props.showMenu !== Constants.DEPARTURES_MENU.ENABLED ?
                        <View style={{width: Dimensions.get('window').width - 30}}>
                            ...
                            <View style={styles.searchContainer}>
                                <TouchableWithoutFeedback
                                    onPress={() => props.changeDeparturesSelectedStop(Constants.DEPARTURES.SELECTED_START_STOP_INPUT)}>
                                ...
                                <TouchableWithoutFeedback
                                    onPress={() => props.changeDeparturesSelectedStop(Constants.DEPARTURES.SELECTED_DESTINATION_STOP_INPUT)}>
                                    <View style={styles.row}>
                                        <View style={styles.flexRow}>
                                            <Image source={!isSelectedStartStop ? dot : null} style={styles.img}/>
                                ...
                                <TouchableOpacity
                                    ...
                                    activeOpacity={0.6}>
                                    <Text style={{textAlign: 'center', fontSize: 18, color: '#fff'}}>Zobraziť
                                        odchody</Text>
                                </TouchableOpacity>
                            </View>
                                    ...
                                    <View style={styles.departuresContainer}>

                                        {
                                            props.departures.length > 0 ?
                                                <ScrollView>
                                                    {
                                                        props.departures.map((departureRow, i) => <DeparturesRow key={i}
                                                                                                                 data={departureRow}/>)
                                                    }
                                                </ScrollView>
